...
TEST(EventLoop, ProcessEventsDoesNotLeaveDanglingThreadDefault) {
    ...
    sender s;
    receiver r;
    s.value_changed.connect(&r, &receiver::on_value, utils::connection_type::direct);
...
TEST(EventLoop, DelayedTimerHonorsBudgetAndCancel) {
    ...
    auto id = loop.post_delayed(std::chrono::milliseconds(20), [&] { value = 2; });
...
TEST(SignalAndSlots, QueuedEmitRunsOnWorkerThread) {
    ...
    s.value_changed.connect(r.get(), &receiver::on_value, utils::connection_type::queued);
