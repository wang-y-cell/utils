...
| 项 | 说明 |
|----|------|
| 头文件 | [`component/signal_and_slots/signal_and_slots.h`](../component/signal_and_slots/signal_and_slots.h) |
| 命名空间 | `utils` |
| 依赖 | [`component/result/expected.h`](../component/result/expected.h)（`result<T>`） |
| 可运行示例 | `demo/signal_and_slots/sample.cpp`（目标：`demo_signal`） |
...
- 实现：`component/signal_and_slots/signal_and_slots.h`
- 示例：`demo/signal_and_slots/sample.cpp`
- 错误类型：`component/result/expected.h`（`result` / `result_ok` / `result_err`）
