...
| 项 | 说明 |
|----|------|
| 核心头 | [`adapter/executor/executor.h`](../adapter/executor/executor.h)（在 `utils.h` 中） |
| 适配头 | [`adapter/executor/adapters.h`](../adapter/executor/adapters.h)（**需单独 include**） |
| 命名空间 | `utils` |
| 示例 | `demo/executor/demo.cpp`（`demo_executor`） |
...
1. 使用线程池 / event_loop 适配时必须 `#include "adapter/executor/adapters.h"`。
